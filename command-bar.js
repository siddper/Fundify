...
document.addEventListener('DOMContentLoaded', function() {
  ...
  input.addEventListener('input', function() {
    ...
    filtered = commands.filter(cmd => {
      ...
      // Check if all search terms match the command
      return searchTerms.every(term => {
        // 1. Direct substring match
        if (cmdName.includes(term)) return true;
        
        // 2. Acronym/initial letter matching
        const words = cmdName.split(/\s+/);
        const initials = words.map(word => word.charAt(0)).join('');
        if (initials.includes(term)) return true;
        
        // 3. Word order independence - check if all characters in term appear in order
        let termIndex = 0;
        ...
        // 4. Fuzzy matching with word boundaries
